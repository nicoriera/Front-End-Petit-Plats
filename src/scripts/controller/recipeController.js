  ...
  handleTagSelected() {
    ...
    for (let tag of listOfAllTags) {
      tag.addEventListener("click", () => {
        ...
        const isInTags = this.selectedTags.some(
          (tag) => tag.value === this.tagToDisplay
        );
        ...
        if (
          this.model.getRecipesFilteredBySearchAndTags(
            this.tagToDisplay,
            keywordArray
          )
        ) {
    ...
    if (this.selectedTags.length === 0) {
      ...
      this.selectedTags.forEach((tag) => {
        filteredRecipes = filteredRecipes.filter((recipe) =>
          this.model
            .getRecipesFilteredBySearchAndTags(tag.value, tag.type)
            .includes(recipe)
        );
      });
      this.view.displayRecipesList(filteredRecipes);
